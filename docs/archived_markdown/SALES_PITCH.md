...
Why we win vs. ΓÇ£that new toolΓÇ¥
- Franchise-first architecture: campaigns, media, and schedules cascade across locations with guardrails, not chaos.
- POS-native control: managers trigger or pause campaigns inside the POSΓÇöno extra logins or browser tabs during rush hour.
...
- Orientation & consistency
  ...
  - Same creative, same playback order and durations across all storesΓÇöno ΓÇ£store-by-store drift.ΓÇ¥
- Runtime flexibility for IT
  - Change the backend server URL at runtime on both TV and Manager appsΓÇöno rebuilds. Ideal for static IP moves or new regions.
...
- Week 1: Provision Docker stack; connect to your domain/IP; whitelist Manager app; pilot 2ΓÇô3 stores.
- Week 2: Creative migration and campaign mapping by region/store type; train managers on the POS tab.
- Week 3ΓÇô4: Roll out to remaining stores; enable analytics dashboards for weekly ops and marketing.

Proof points and objections handling
- ΓÇ£We need consistency across 50+ locations.ΓÇ¥ Our cascade model ensures a single source of truth; screens self-heal to the assigned campaign.
- ΓÇ£IT changes IPs sometimes.ΓÇ¥ Both apps support runtime base URL changesΓÇöno device swaps.
- ΓÇ£WeΓÇÖre worried about training.ΓÇ¥ Activation is QR-based; daily tasks live inside the POS where staff already work.
- ΓÇ£What about large video files?ΓÇ¥ Assets are streamed with caching; orientation rules prevent stretched or cropped content.

Packaging and pricing (example)
- SaaS per-screen monthly with volume tiers (e.g., 1ΓÇô25, 26ΓÇô100, 101+). Includes hosting, updates, and support.
...
Call to action
- Pick 3 pilot stores. WeΓÇÖll activate them in under an hour and show you live, synchronized promosΓÇöno print, no waiting.
